  ...
  const columns: ColumnDef<TableData>[] = [
    {
      ...
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
            className={`w-full justify-start font-semibold text-[15px] mb-1 ${column.getIsSorted() ? "text-indigo-600 dark:text-primary" : "text-foreground"}`}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="cursor-pointer mr-2 flex-shrink-0">
                  <ExternalLink
                    size={16}
                    className="text-current hover:text-indigo-600 dark:hover:text-primary"
                    onClick={(e) => {
                      e.stopPropagation();
                      window.open(
